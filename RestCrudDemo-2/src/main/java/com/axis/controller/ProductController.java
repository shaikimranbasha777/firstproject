...
public class ProductController {
	...
	ResponseEntity<Product> getProductByName(@PathVariable String productName){
		return new ResponseEntity<Product>(productService.getProductByName(productName), HttpStatus.OK);
	}
	
	@GetMapping("/productbylessthenproductprice/{productPrice}")
	ResponseEntity<List<Product>> getProductLessthenProductPrice(@PathVariable int productPrice){
		return new ResponseEntity<List<Product>>(productService.getProductPriceLessThen(productPrice), HttpStatus.OK);
	}
	
	@GetMapping("/productbygreaterthenproductprice/{productPrice}")
	ResponseEntity<List<Product>> getProductGreaterthenProductPrice(@PathVariable int productPrice){
		return new ResponseEntity<List<Product>>(productService.getProductPriceGreaterThen(productPrice), HttpStatus.OK);
	}
}
